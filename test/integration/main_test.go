...
// Запускается до тестов (например для поднятия коннекшена к базе).
...
func (s *Suite) TearDownSuite() {} // Запускается вконце, после тестов и закроет коннекшн к базе данных.

func (s *Suite) SetupTest() {} // Запускается каждый раз перед тестом (например прогоняются миграции).

func (s *Suite) TearDownTest() {} // Запускается каждый раз после теста (например очистить базу данных).
